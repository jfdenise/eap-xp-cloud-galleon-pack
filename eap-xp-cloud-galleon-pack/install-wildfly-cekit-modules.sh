...
pushd "$tmp_dir" > /dev/null
  ...
  pushd wildfly-cekit-modules/export
    sh ./export-cekit-modules.sh "$resources_dir" "$SCRIPT_DIR/wf-cekit-modules-fp-content-list.txt" "$SCRIPT_DIR/wf-cekit-modules-launch-list.txt" "$SCRIPT_DIR/wf-cekit-modules-launch-config-list.txt"
  popd > /dev/null
popd > /dev/null
